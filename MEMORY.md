...
| **后端边界决策（以下 4 项）** | ✅ **已定稿** | 详细文档见 doc/specs/backend-boundaries.md，含三阶模型、全量事件映射表、模糊地带判定规则 |
| Beancount 接入方式 | ✅ 已定稿 | Task Executor import `beancount_writer.py` 共享库，写入隔离文件 `homebus-{MM}.bean`，幂等 + bean-check + auto git commit。详见 doc/specs/beancount-integration.md |
