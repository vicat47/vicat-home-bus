...
- [ ] CLI 提供 `homebus publish` 命令，接受 `--body` JSON 参数（或 `--file` 从文件读取）
...
- [ ] v0.1 Beancount 查询仅支持 `verify_entry`（按 event_id 验证写入），完整查询（余额/账目）v0.2 走 Fava
...
Agent 收到用户输入 → 意图解析 + 实体提取
         │
         ▼
    terminal("homebus publish --body '{\"intent\":\"purchase\",\"items\":[...],\"total_price\":180}'")
         │
         ▼
...
| `homebus/adapters/beancount_writer.py` | Beancount 共享库（文件I/O、bean-check、git） |
| `homebus/registry.py` | 路由注册表（TOML 加载 + 缓存 + 查询） |
...
|------|---------|---------|
| Grocy | 4.x+ | REST API |
| Beancount | 2.x+ | 写入：import 共享库（beancount_writer.py）；读取（v0.1）：文件扫描 verify_entry；读取（v0.2）：Fava REST API |
...
2. **Beancount 接入方式** — ✅ 已决策：Task Executor import `beancount_writer.py` 共享库（幂等 + bean-check + git commit），读取走 Fava REST API（v0.2）
   → 详见 [beancount-integration.md](../specs/beancount-integration.md)

3. **CLI 的 JSON 参数如何安全传递** — ✅ 已决策：`--body` 单 JSON 方案 + `--file` 备选
   → 详见 [cli-spec.md](../specs/cli-spec.md)
