...
| **v0.2** | Agent 体验升级 | MCP Server + Webhook 回调 | v0.1 后 1-2 周 |
...
v0.1 ─────────────────────────────────────────
  │                                              │
  ├→ v0.2 (MCP + Webhook, 层叠在 API 之上)         │
  │                                                 │
  ├→ v0.3 (调谐, 依赖 events 日志和 Adapters)       │
  │     │                                           │
  │     └→ v0.4 (物化视图, 依赖调谐的数据同步)      │
  │                                                 │
  └──────────────────────→ v1.0 (HA 等, 在 v0.4 之后)
...
v0.1 是地基；v0.2 和 v0.3 可以并行；v0.4 依赖 v0.3；v1.0 依赖所有前面阶段。
